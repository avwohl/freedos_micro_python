...
patch_main_pktdrv_prealloc() {
    ...
    awk '
        ...
        /^int main\(int argc, char \*\*argv\) \{/ && !inserted {
            ...
            print "    regs[1] = 200;          // 200 paragraphs (~3.1 KB) — fits 2 packet slots"
            ...
            print ""
            print "    // Pre-allocate 8 paragraphs (128 bytes) for the INT 0x60 thunk."
            print "    // First 16 bytes are the INT 0x60 trampoline + ARP/IPv4/IPv6"
            print "    // type filter bytes; offsets 0x10..0x6F hold the 2-slot RX"
            print "    // receiver stub (bypasses DOS/32A's broken DPMI 0x0303"
            print "    // dispatch); offsets 0x70..0x7B hold per-slot pending/length"
            print "    // flags + the two bounce-buffer segments.  Two slots are"
            print "    // required so that back-to-back TX-ACK and Finished frames"
            print "    // (200-500us apart) both reach lwIP — single-slot would drop"
            print "    // one and hang the TLS handshake.  See pktdrv_alloc_thunk in"
            ...
            print "    rt[1] = 8;               // BX = 8 paragraphs (128 bytes total)"
